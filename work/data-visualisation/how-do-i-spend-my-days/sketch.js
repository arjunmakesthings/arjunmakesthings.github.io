...
let aspectRatio = [108, 192]; //width and breadth

let htmlContainer = document.getElementById('canvas-container-but-hidden-on-phone'); 
let htmlContainerWidth = htmlContainer.offsetWidth; 
let htmlContainerHeight = ((htmlContainerWidth *aspectRatio[0]) / aspectRatio[1]); 

var canvas = createCanvas (htmlContainerWidth,htmlContainerHeight); 
canvas.parent('canvas-container-but-hidden-on-phone'); 
  
  if (htmlContainerWidth<600){
    headSize = 56; 
    let tSize = 8; 
  }else{
   headSize = 80; 
  ...
  particleSize = htmlContainerWidth/40; 
